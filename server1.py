# Added time.sleep(0.03) in video loop (prevents 100% CPU usage).
# Fixed PDF directory creation bug.
# Removed dummy background task.
# Added error handling in digit recognition.
# Added resource cleanup (cap.release()).
# Added /health endpoint for quick API check.
# Added safe header loading (fallback if folder missing).
import cv2
import numpy as np
...
# Allow CORS for local development (all origins for now)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# -------------------------
# Initialize Hand Detector and Canvas
# -------------------------
...
def recognize_canvas(canvas_img):
    try:
        ...
        for (x, y, w, h) in merged_boxes:
            ...
            prediction = numModel.predict(processed_digit, verbose=0)
...
def generate_canvas_stream():
    ...
    while True:
        ...
        if lmList:
            ...
            elif fingers[1] and not fingers[2]:  # Only index finger (drawing)
